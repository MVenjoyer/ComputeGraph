...
def add_word_count(rows: TRowsIterable, text_column: str = "text_column") -> TRowsGenerator:
    """
...
def yandex_maps_graph(input_stream_name_time: str, input_stream_name_length: str,
    ...
    return Graph.graph_from_iter(input_stream_name_time) \
        ...
        .map(operations.Evaluate(lambda row: dis_iso(row, enter_time_column, leave_time_column,
                                                     "%Y%m%dT%H%M%S.%f", weekday=weekday_result_column,
                                                     hour=hour_result_column))) \
